...
impl SimdValidator {
    ...
    pub fn find_at(email: &str) -> Option<usize> {
        ...
        while i + 8 <= len {
            ...
            let has_at = (xor.wrapping_sub(0x0101010101010101u64) & !xor & 0x8080808080808080u64) != 0;
    ...
    pub fn count_at(email: &str) -> usize {
        ...
        while i + 8 <= len {
            ...
            // SWAR: count @ in parallel
            let eq = chunk ^ 0x4040404040404040u64;
            let matches = (eq.wrapping_sub(0x0101010101010101u64) & !eq & 0x8080808080808080u64);
        ...
        // Remaining bytes
        count += bytes[i..].iter().filter(|&&b| b == b'@').count();
        count
    }
